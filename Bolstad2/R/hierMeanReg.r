...
#' 
#' @export hierMeanReg
hierMeanReg <- function(design, priorTau, priorPsi, priorVar, priorBeta = NULL, steps = 1000, startValue = NULL, randomSeed = NULL) {
    ## design is expected to be a list with elements, y = response vector, group = grouping vector, x = matrix of covariates or null

    ## priorTau is a list with elements tau0 and v0 priorPsi is a list with elements psi0 and eta0 priorVar is a list with elements s0 and kappa0 priorBeta is a list with
    ## elements b0 and bMat or null if x is null startValue is a list with possible elements tau, psi, mu, sigma and beta
    ...
    for (n in 2:steps) {
        ...
        for (j in 1:nGroups) {
            varMu <- (sigmaSqSample[n - 1] * psiSample[n - 1])/(sigmaSqSample[n - 1] + nj[j] * psiSample[n - 1])
            xBeta <- 0
            if (bReg)
    ...
    if (bReg) {
        results.df <- data.frame(tau = tauSample, psi = psiSample, mu = muSample, beta = betaSample, sigmaSq = sigmaSqSample, sigma = sigmaSample)
    } else {
        results.df <- data.frame(tau = tauSample, psi = psiSample, mu = muSample, sigmaSq = sigmaSqSample, sigma = sigmaSample)
    }
    describe(results.df)
